...
axios.interceptors.request.use(
  config => {
    ...
    //   config.url = `${config.url}?token=${store.state.mainData.token}`;
    // }
    let token = store.state.mainData.token || ss.getItem('token') || '';
    if (config.url !== 'https://up.qbox.me/') {
    ...
    if (status !== 0) {
      ...
      if (code === '401') {
        ...
        if (browserPath.indexOf('.com') > -1 || browserPath.indexOf('.cn') > -1) {
          window.location.href = 'http://192.168.15.215:9090/sso/checkLogin';
        } else {
          window.location.href = BUTILS.getUrl();
        }
        store.dispatch('loginStatuActions', false); // 退出登录状态

...
<template>
  <div>
    <el-dialog class="vue-utils-Dialog vuePopupUpload" title="选择文件" append-to-body :visible.sync="vuePopupUploadShow" width="650px" :close-on-click-modal="false">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button class="f-but-blue" :disabled="isDisabled ? isDisabledUploadFile : uploadFileDisable" @click="fileUploadClick">确定</el-button>
...
export default {
  ...
  methods: {
    ...
    picupload(event) { // 向后端获取upToken
      ...
      if (file) {
        ...
        if (isName) {
          ...
          //   formData['key'] = `${res.model.fileName}/${fileNames}.${fileSuffix}`
          this.picuploadPhoto(file, formData, fileNames, fileSize, fileSuffix)
          // })
        } else {
          return false
        }
    ...
    picuploadPhoto(file, formData, fileNames, fileSize, fileSuffix) { // 上传图片至七牛
      let params = { file }
      ...
      this.$axiosUpload(fileLoad_uploadRequest, params).then(res => {
        if (res.result === 'success' && res.model) {
          // this.fileList[this.fileList.length - 1].url = this.upUrl + res.key
          this.fileList[this.fileList.length - 1].url = res.model.path;
          this.uploadFileDisable = false
          this.isDisabledUploadFile = false
        } else {
          this.$store.commit('uploadProgressMutations', 0)

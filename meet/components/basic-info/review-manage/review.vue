...
export default {
  ...
  methods: {
    // 获取表单基础数据
    getFormData() {
      this.$axiosGet(API.verifyConfig_toAddApplyVerifyConfig, {}).then(res => {
        this.formData.userList = res.data.userList || [];
        this.formData.custormerOrganList = res.data.custormerOrganList || [];
      });
    },
    // 选中节点
    ...
    getDetail(id) {
      ...
      this.$axiosGet(API.verifyConfig_toUpdateApplyVerifyConfig, params).then(
        res => {
          this.childNodeList = res.data.custormerOrgans;
        }
      );
    },
    // 提交
    submitClick() {
      this.$refs.ruleForm.validate(valid => {
        ...
        if (this.$route.params.type == "edit") {
          ...
          url = API.verifyConfig_doUpdateApplyVerifyConfig;
          this.staffId = this.ruleForm.reviewId;
          this.userName = this.ruleForm.reviewName;
        } else {
          url = API.verifyConfig_doAddApplyVerifyConfig;
        }
        if (valid) {
          let params = {
            organIds: this.childNodeKeys.join(","),

        ...
        >
          ...
          <tbody @mousedown="dragMousedown($event)">
            <tr v-for="(item, index) in list" :key="index">
              <td
                :rowspan="sysBoardConfig.length"
                v-if="item.name"
                class="calendar-picker"
              >{{item.currentDay}}</td>
              <td class="calendar-begin">{{item.begin}}</td>
              <td
                v-for="(li, i) in item.li"
                :key="i"
                :rowspan="li && li.rangeIndex ? li.rangeIndex : 1"
                v-show="li"
                :class="li && !li.order ? 'fileDiv' : ''"
                :date-meet="roomInfoPageList[i].name"
                :data-name="pickerName(index)"
                :data-period="item.begin"
                :data-id="li ? li.id : ''"
              ...
              >
                <div
                  ...
                  @click="applyFormDetail(li.order.id)"
                >
                  <el-tooltip placement="bottom" effect="light">
                    <div slot="content" class="tooltip-content">
                      <div class="tooltip-content-organ">申请单位：{{li.order.organName}}</div>
                      <div
                        class="tooltip-content-apply"
                      >申请人：{{li.order.applyUserName}} {{li.order.applyUserPhone}}</div>
                    </div>
                    <div
                      class="calendar-booking-div"
                      :class="li.order.state === 'NEW_APPLY' || li.order.state === 'USER_CHECK_SUCCESS' || li.order.state === 'MANAGE_CHECK_SUCCESS' ? 'fail-state' : ''  "
                      v-if="li.order.state"
                    >
                      <span>{{handleBoardStatus(li.order.state,li.order.applyUserId)}}</span>
                    </div>
                  </el-tooltip>
                </div>
              </td>
...
export default {
  ...
  data() {
    return {
      ...
      boardStatusText: "" // 看板返回文字修改
    };
  },
  activated() {
    this.search.startTime = DATE.currentDate();
    this.search.endTime = new Date(
      DATE.stringToDate(DATE.currentDate()).getTime() + 6 * 24 * 60 * 60 * 1000
    );
    this.getDataRequest();
    this.confirmClose();
    // console.log('loginStatu',this.loginStatu.userId)
  },
  computed: {
    ...mapState("testData", ["meetBoardTestDate"]),
    ...mapState("mainData", ["loginStatu"])
  },
  methods: {
    ...mapActions("mainData", [
      "burningTimeActions",
    ...
    getDataRequest() {
      ...
      // 测试数据
      // let res = this.meetBoardTestDate;
      // if (res.data) {
      //   this.roomInfoPageList = res.data.roomInfoPageList || {};
          ...
          if (data.key[roomInfoPageList[i].id]) {
            ...
            for (let j = 0, lenj = value.length; j < lenj; j++) {
              if (begin === value[j].begin) {
                ...
                list[i] = value[j];
                if (value[j].rangeIndex > 1)
                  this.rangeIndex.push({
                    id: roomInfoPageList[i].id,
                    index: value[j].rangeIndex
                  });
                break;
              }
            }
        ...
        for (let i = 0, len = boardList.length; i < len; i++) {
          ...
          if (i === 0) {
            list.push({
              ...
              room: roomList[i] ? roomList[i].name : "",
    ...
    getPeriod(index) {
      let rIndex = 1,
        num = this.sysBoardConfig.length;
        ...
        if (res && res.length > 0) {
          ...
          if (res.length > 1) {
            if (res[0].name === res[res.length - 1].name) {
              context = `${res[0].name}&ensp;${res[0].start}&ensp;~&ensp;${
                res[res.length - 1].end
              }`;
            } else {
              context = `${res[0].name}&ensp;${res[0].start}&ensp;~&ensp;${
                res[res.length - 1].name
              }&ensp;${res[res.length - 1].end}`;
            }
          }
    ...
    confirmClose() {
      ...
      for (let value of fileNodes) {
        ...
        if (value.className.indexOf("fileDiv") !== -1)
          value.className = "fileDiv";
      }
    },
    // 查询
    searchClick() {
    ...
    roomInfoDetail(itemID) {
      let meetRoomList = this.roomInfoPageList.list;
      this.$refs.meetRoomDetail.title = "会议室详情";
      this.$refs.meetRoomDetail.openModal(itemID);
      this.$refs.meetRoomDetail.roomInfoImgList = meetRoomList[itemID].attachImg
        ? meetRoomList[itemID].attachImg.split(",")
        : ["1", "2", "3"];
      this.$refs.meetRoomDetail.roomInfoListDetail = meetRoomList[itemID];
    },
    // 会议室详情弹窗关闭
    confirmMeetClose() {
    ...
    // 看板状态文字处理
    handleBoardStatus(status, applyUserId) {
      let handelText = "";
      let userId = this.loginStatu.userId;
      if (applyUserId === userId) {

...
<template>
  <div style="overflow: hidden;">
    <div v-for="(li, index) in pictureList" :key="index" class="upload-main-div" style="width: 140px; height: 138px;">
      <img :src="li.url" class="upload-img" alt="" style="max-width: 200px; max-height: 200px;" />
      <input v-if="!disabled" class="upload-btn" title=' ' type="file" @change="picupload($event, index)" :id="acceptId + index" style="width: 142px; height: 140px;">
      <div class="title" v-show="!li.url" style="width: 142px; height: 140px;">
        <div style="width: 142px; height: 140px;">
          <p><i class="iconfont iconjiahao"></i></p>
  ...
  methods: {
    picupload(event, index) { // 向后端获取upToken
      ...
      if (file) {
        ...
        if (isName) {
          // self.$axiosPost(API.fileLoad_uploadRequest, {}).then(res => {
    ...
    picuploadPhoto(file, index) { // 上传图片至七牛
      ...
      this.$axiosForm(API.file_uploadFileToServer, params, (schedule, index) => {
      }).then(res => {
        ...
        if (res.status === 200 && res.data) {
          this.pictureList[index].url = res.data.url;
          this.pictureList[index].name = res.data.fileName;
          if (this.pictureList.length < this.maxNumber) {

<template>
  <div id="base-upload">
    <el-upload
      ...
      <el-button slot="trigger" type="warning" icon="el-icon-upload"><i class="icon iconfont icon-daochu"></i>上传附件</el-button>
...
export default {
  ...
  methods: {
    ...
    handleSuccess(response, file, fileList) {
      if (response.status === 200 && response.data) {
        ...
        this.fileList = fileList.map(item => {
          if (item.id) {
            return {fileName: item.name, attachUrl: item.url, id:item.id}
          } else {
            return {fileName: item.name, attachUrl: item.response.data ? item.response.data.url : '',}
          }
        });
        // console.log('文件上传成功 fileList',this.fileList)
    ...
    handleRemove(file, fileList) {
      ...
      this.fileList = fileList.map(item => {
        if (item.id) {
          // this.deleteApplyAttachment(item.id)
          return {fileName: item.name, attachUrl: item.url,}
        } else {
          return {fileName: item.name, attachUrl: item.response.data.url,}
        }
        if (item.url) {

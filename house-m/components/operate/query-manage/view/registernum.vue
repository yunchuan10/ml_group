...
export default {
  ...
  computed: {
    operateOrganId() {
      return this.$store.state.mainData.operateOrganId;
    },
    organName() {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$axiosGet(API.comprehensive_getRentalPage, params).then(res => {
        if (res.data) {
          let list = [];
    ...
    getCombarlinedata() {
      ...
      this.$axiosGet(API.comprehensive_getRentEchart, params).then(res => {
        if (res.data) {
          this.hosueRentList = res.data
          let roomUseList = [];
          let legend = [];
          if (this.hosueRentList.echartRentResponseList) {
            roomUseList.push({
              value: this.hosueRentList.echartRentResponseList[0].totalRentalMoney || '0',
              name: `已入账租金`
            })
          }
          if (this.hosueRentList.echartRentOverDueResponseList) {
            roomUseList.push({
              value: this.hosueRentList.echartRentOverDueResponseList[0].totalRentalMoney || '0',
              name: `已逾期租金`
            })
          }
          if (this.hosueRentList.echartRentDueResponseList) {
            roomUseList.push({
              value: this.hosueRentList.echartRentDueResponseList[0].totalRentalMoney || '0',
              name: `即将到期租金`
            })
          }

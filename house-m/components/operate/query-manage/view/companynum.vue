          ...
          <tr>
            <td>
              <ul class="f-clearfix" @keydown="$keydown($event, getSearch)">
                ...
                <li class="radio-group" style="padding-top:10px">
...
<script>
import roomDetail from '@/components/operate/room-manage/room-companynum'; // 详情弹出框
...
export default {
  ...
  methods: {
    ...
    getCombarlinedata() {
      ...
      this.$axiosGet(API.comprehensive_getContractEchart, params).then(res => {
        if (res.data) {
    ...
    getDataRequest() {
      ...
      this.$axiosGet(API.comprehensive_getContractPage, params).then(res => {
        if (res.data) {
          let list = [];
          list = res.data.data || [];

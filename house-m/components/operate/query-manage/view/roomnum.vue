  ...
  <div>
    <div class="h-table">
          ...
          <tr>
            <td>
              <ul class="f-clearfix" @keydown="$keydown($event, getSearch)">
                <li>
                  <span>街道</span>
                  <el-input clearable filterable placeholder="请输入" v-model="street"></el-input>
                </li>
                <li>
                  <span>门牌号</span>
                  <el-input clearable filterable placeholder="请输入" v-model="houseNo"></el-input>
                </li>
                <li>
                  <span>取得方式</span>
      ...
      <el-table :data="list" stripe style="width: auto">
        ...
        <el-table-column prop="picture" label="照片信息">
          <template slot-scope="scope">
...
<script>
import roomDetail from '@/components/operate/room-manage/room-roomnum'; // 详情弹出框
export default {
  components: {
  ...
  methods: {
    ...
    getRoomCount() {
      ...
      this.$axiosGet(API.comprehensive_getHouseEchart, params).then(res => {
        if (res.data) {
    ...
    getTableData() {
      ...
      this.$axiosGet(API.comprehensive_getHousePage, params).then(res => {
        if (res.data) {
          let list = [];
          list = res.data.data || [];

  ...
  <div>
    <section class="m-main f-clearfix" style="height: calc(100% - 35px);box-sizing:border-box;">
      ...
      <div class="m-breadcrumb u-bread">
        ...
        <div class="f-pull-left f-weight-bold f-size">{{street || ''}}
          <!-- <span class="f-weight-normal" v-show="courtyardAddress? true:false">({{courtyardAddress}})</span> -->
        </div>
        <div class="u-bread-btn-group">
          <router-link to="/operate/query-manage-street/query-street-list" v-show="!isShowLeftBannel">
      ...
      <div style="height: calc(100% - 61px);display:flex">
        <div class="u-treecontent" style="height: 100%;" v-if="isShowLeftBannel">
          <div class="u-tree-seachbox">
            <el-input placeholder="请输入街道名称" v-model="filterText">
            </el-input>
          </div>
          <div style="height: calc(100% - 110px);overflow: auto;" class="treeIconStyle">
            <el-tree class="filter-tree" @node-click="updateCourtyardName" :highlight-current="true" node-key="value" :data="roomNoteList" :props="defaultProps" :accordion="true" :filter-node-method="filterNode" ref="tree2" :render-content="renderContent">
            </el-tree>
          </div>
        </div>
        <div class="u-left" style="height: 100%;overflow: auto;margin: 0;flex: 1;padding-left:20px;">
          <el-row :gutter="20">
            <el-col :span="8">
              <router-link :class="iconShowlist[0]?'router-link-exact-active router-link-active':''" :to="{path: '/operate/query-manage-street/query-street-list/roomnum', query: {organId: $route.query.organId, street: $route.query.street}}" name="roomNum">
            ...
            <el-col :span="8">
              <router-link :class="iconShowlist[1]?'router-link-exact-active router-link-active':''" :to="{path: '/operate/query-manage-street/query-street-list/companynum', query: {organId: $route.query.organId, street: $route.query.street}}" name="companyNum">
            ...
            <el-col :span="8">
              <router-link :class="iconShowlist[2]?'router-link-exact-active router-link-active':''" :to="{path: '/operate/query-manage-street/query-street-list/registernum', query: {organId: $route.query.organId, street: $route.query.street}}" name="registerNum">
...
export default {
  ...
  activated() {
    if (this.$route.query.street) {
      ...
      this.$store.dispatch('operateStreetActions', this.$route.query.street);  //经营性房产 综合查询按单位id
      this.getYardata();
    } else {
      this.isShowLeftBannel = true;
      this.getRoomTree();
      this.$store.dispatch('operateStreetActions', '');
      this.name = ''
    }
  ...
  watch: {
    ...
    '$route.query.street': function (value) {
      if (!value) {
        this.isShowLeftBannel = true;
        this.getRoomTree();
        this.$store.dispatch('operateStreetActions', '');
  ...
  methods: {
    ...
    getRoomTree() {
      ...
      this.$axiosGet(API.comprehensive_getStreetListByRequest, params).then(res => {
        if (res.data.length > 0) {
          this.roomNoteList = res.data;
          this.organId = res.data[0].value;
          this.courtyardName = res.data[0].label;
          this.street = res.data[0].label;
          this.$nextTick(() => {
    ...
    getYardata() {
      ...
      this.$axiosGet(API.comprehensive_getStreetTotalData, params).then(res => {
        if (res.data) {
          this.houseNum = res.data.totalHouseNum || '0';
          this.contractNum = res.data.totalContractNum || '0';
          this.rentNum = res.data.totalRentNum || '0';
        } else {
          this.houseNum = '0';
          this.contractNum = '0';
          this.rentNum = '0';
        }
        this.$store.dispatch('operateStreetActions', this.street);
      })
    },
    updateCourtyardName(data) {
      if (data.typeFlag === 'street') {
        this.organId = data.value;
        // this.courtyardName = data.label;
        this.street = data.label;
        this.getYardata();
      }
    }
  },
...
.treeIconStyle .el-tree-node__expand-icon::before {
  display: none;
}
</style>

        ...
        <div class="h-tab-content" v-show="pageSelect=='basic'">
          <table class="h-table-info">
            ...
            <tr>
              ...
              <td>{{contractDetail.rental || ''}}
                <span v-show="contractDetail.rental != ''">元/年</span>
              </td>
            </tr>
            <tr>
    ...
    getDataRequestBasics() {
      ...
      this.$axiosGet(API.contract_getContractDetail, params).then(res => {
        if (res.data) {
          ...
          if( res.data.rentalInformationResponseList) //租金详情
          {
            this.list = res.data.rentalInformationResponseList; //租金详情
            if (res.data.rentalInformationResponseList.startDate) {
              this.rentStartDate = FORMATGET.formatGET(new Date(res.data.rentalInformationResponseList.startDate));
            } else {
              this.rentStartDate = '';
            }
            if (res.data.rentalInformationResponseList.endDate) {
              this.rentEndDate = FORMATGET.formatGET(new Date(res.data.rentalInformationResponseList.endDate));
            } else {
              this.rentEndDate = '';
            }
          }else{

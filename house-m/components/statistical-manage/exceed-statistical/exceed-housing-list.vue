...
export default {
  ...
  activated() {
    if (this.$route.query) {
      ...
      this.roomTypeIds = this.$route.query.roomTypeIds;
    }
    if (this.roomTypeIds == 'SERVICEHOUSE') {
      this.type = '服务用房'
    } else if (this.roomTypeIds == 'EQUIPHOUSE') {
      this.type = '设备用房'
    } else if (this.roomTypeIds == 'AFFILIATEDHOUSE') {
      this.type = '附属用房'
    }
    this.roomLocation = [];
    this.addressId = '';
  ...
  methods: {
    ...
    getDataRequest() {
      let params = {
        ...
        roomTypeId: this.roomTypeIds || [],
        useOrganId: this.organId,
        pageNum: this.page,
        pageSize: this.pageSize
      };
      this.$axiosGet(API.excessvie_excessiveOtherRoomList, params).then(res => {
        let list = [];
        if (res.data) {
          list = res.data.data;

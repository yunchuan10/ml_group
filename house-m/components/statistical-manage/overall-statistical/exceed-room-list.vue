      ...
      <div class="h-table f-relative">
        ...
        <el-table :data="list" stripe style="width: 100%">
          ...
          <el-table-column prop="roomNo" label="房间号">
            <template slot-scope="scope">
              <a @click="basicInfoClick(scope.row.roomId,type,scope.row.excessive)">{{scope.row.roomNo}}</a>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="userName" label="当前使用人">
            <template slot-scope="scope">
...
<script>
import basicDetail from './view/room-record-detail'; // 详情弹出框
export default {
  components: {
  ...
  methods: {
    ...
    getDataRequest() {
      ...
      this.$axiosPost(API.excessvie_excessiveOfficeRoomList, params).then(res => {
        let list = [];
        if (res.data) {
          list = res.data.data || '';
          this.list = list;

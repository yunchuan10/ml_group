<template>
          ...
          <div v-show="!isShow" class="plane-box-info">           
            <ul class="box-info-ul">
              <li v-for ="(item,index) in list" :key="index">
                <a  @click="getDetailClick(index, item.roomId)" :class="[index_R === index?'active':'',item.houseRoomPoints?'bind-color':'']"  :title="item.roomNo.length > 4?item.roomNo:''">
        ...
        <div class="plane-box f-clearfix">
          ...
          <div v-show="!isDetail" class="plane-box-info">           
            <table class="box-info-tab">
              ...
              <tr><td>使用面积</td><td class="td-color">{{roomData.userArea ? roomData.userArea+'㎡': ''}}</td></tr>
              <tr><td>照片信息</td><td > 
  ...
  <!-- 画布右键出现的按钮 -->
    ...
    <!-- 新增房间 -->
    <room-update ref="roomUpdate" @requestData="getDataRequest"></room-update>  
    <!-- 取消关联房间操作提示框 -->
...
export default{
  ...
  methods:{
    ...
    relativeDataCallBack(data) {
      ...
      let roomNo = new fabric.Textbox(`${data.roomNo}\n(${data.userArea}㎡)`, {
        left: this.selectOneObj.type == 'ellipse'? this.selectOneObj.left - 30 : this.selectOneObj.width/2 + this.selectOneObj.left - 30,
        top: this.selectOneObj.type == 'ellipse'? this.selectOneObj.top - 10 : this.selectOneObj.height/2 + this.selectOneObj.top,
        width: 100,
        fontSize: 14,
        fill: this.drawColor,
        hasControls: false,
    ...
    cancelRelaData(){
      ...
      if (fabricObjone.type == 'rect') {
        fabricObject = new fabric.Rect({
          ...
          width: this.selectOneObj.width,
          height: this.selectOneObj.height,
    ...
    getCopyFloorInfo() {
      ...
      this.$axiosPost(API.plan_listRoom, params).then(res => {
        if (res.result == 'success' && res.data) {
          res.data.roomList.data.forEach(item => {
            if (item.houseRoomPoints) {
              let cavObj = JSON.parse(item.houseRoomPoints);
              let fabricObject = null;
              if (cavObj.type == 'rect') {
                fabricObject = new fabric.Rect({
                  left: cavObj.left,
                  top: cavObj.top,
                  stroke: cavObj.stroke,
                  strokeWidth: cavObj.strokeWidth,
                  width: cavObj.width,
                  height: cavObj.height,
                  fill: 'rgba(255, 255, 255, 0)',
                  scaleX: cavObj.scaleX,
                  scaleY: cavObj.scaleY,
                  angle: cavObj.angle
                });
                this.fabricObj.add(fabricObject);
              } else if (cavObj.type == 'circle') {
                fabricObject = new fabric.Circle({
                  left: cavObj.left,
                  top: cavObj.top,
                  stroke: cavObj.stroke,
                  fill: 'rgba(255, 255, 255, 0)',
                  radius: cavObj.radius,
                  strokeWidth: cavObj.strokeWidth,
                  scaleX: cavObj.scaleX,
                  scaleY: cavObj.scaleY,
                  angle: cavObj.angle
                ...
                this.fabricObj.add(fabricObject);
              } else if (cavObj.type == 'ellipse') {
                fabricObject = new fabric.Ellipse({
                  left: cavObj.left + cavObj.rx,
                  top: cavObj.top + cavObj.ry,
                  stroke: cavObj.stroke,
                  fill: 'rgba(255, 255, 255, 0)',
                  originX: cavObj.originX,
                  originY: cavObj.originY,
                  rx: cavObj.rx,
                  ry: cavObj.ry,
                  strokeWidth: cavObj.strokeWidth,
                  scaleX: cavObj.scaleX,
                ...
                this.fabricObj.add(fabricObject);
              } else if (cavObj.type == 'path') {
                let pathString = '';
                cavObj.path.forEach((item, index) => {
                  if (item[0] == 'M' || item[0] == 'L') {
                    pathString += `${item[0]} ${item[1]} ${item[2]} `
                  } else if (item[0] == 'Q') {
                    pathString += `${item[0]} ${item[1]} ${item[2]}, ${item[3]} ${item[4]} `
    ...
    getTree(){
      ...
      this.$axiosPost(API.plan_getCourtyardTree,params).then( res=>{
        if(res.data){
    ...
    getDetailClick(index, roomid) {
      ...
      this.$axiosPost(API.room_queryDetail, params).then(res => {
        if(res.data){
          ...
          if(this.roomData.houseRoomPoints && this.roomData.useStatus!=='IDLE' && this.roomID){
            this.isDelete = true;
          }else{
            this.isDelete = false;
          }
        ...
        if(res.data.fileResponse){
          this.firstImg = res.data.fileResponse[0]?res.data.fileResponse[0].url:'';
        }       
    ...
    getDataRequest(isNeedInitCav) {
      ...
      this.$axiosPost(API.plan_listRoom, params).then(res => {
        ...
        if(this.list){
          ...
          if(this.list[0]){
            this.defaultRoomId = firstData.roomId;
            this.getDetailClick(0,this.defaultRoomId)
          }else{
            this.roomData = {}
          }       
          this.list.forEach((item,index) =>{
            if(item.houseRoomPoints === null || item.houseRoomPoints === ''){
              this.relativeNoData.push(item)
            }else{
              this.relativeData.push(item)
            }
            if (isNeedInitCav && item.houseRoomPoints !== '') {
              let cavObj = JSON.parse(item.houseRoomPoints);
    ...
    initCavObj(roomInfo, cavInfo) {
      ...
      if (cavInfo) {
        ...
        let roomNo = new fabric.Textbox(`${roomInfo.roomNo}\n(${roomInfo.userArea}㎡)`, {
          left: cavInfo.type == 'ellipse'? cavInfo.left - 30 : cavInfo.width/2 + cavInfo.left - 30,
          top: cavInfo.type == 'ellipse'? cavInfo.top - 10 : cavInfo.height/2 + cavInfo.top,
          width: 100,
          fontSize: 14,
          fill: this.drawColor,
          hasControls: false,
    ...
    initCanvas() {
      ...
      //     this.fabricObj.add(JSON.parse(item.houseRoomPoints))
      //     this.fabricObj.setActiveObject(item.houseRoomPoints);
      //   })

            ...
            <el-table :data="tableData" style="width: 100%;  height: 650px; overflow-y:auto;" show-summary>
              <el-table-column prop="areaName" label="地区" width="120">
                <template slot-scope="scope">
                  <router-link v-if="typeName ==='organ'" :to="{path:'/home/homemap/map-company', query: {areaId:scope.row.areaId,areaType: scope.row.areaType}}">{{ scope.row.areaName}}</router-link>
                  <router-link v-if=" typeName === 'yard'" :to="{path:'/home/homemap/map-courtyard', query: {areaId:scope.row.areaId,areaType: scope.row.areaType}}">{{ scope.row.areaName}}</router-link>
                </template>
              </el-table-column>
              <el-table-column prop="organNum" label="单位数量" v-if="typeName =='organ'">
              </el-table-column>
              <el-table-column prop="yardNum" label="院落数量" v-if="typeName =='yard'">
              </el-table-column>
              <el-table-column prop="userAreaSum" label="总使用面积" width="120" v-if="typeName =='yard'">
              </el-table-column>
              <el-table-column prop="buildAreaSum" label="总建筑面积" width="120" v-if="typeName =='yard'">
              </el-table-column>
              <el-table-column prop="idleAreaSum" label="闲置面积" v-if="typeName =='yard'">
              </el-table-column>
              <el-table-column prop="emptyAreaSum" label="空置面积" v-if="typeName =='organ'">
              </el-table-column>
              <el-table-column prop="baseAreaSum" label="基本办公用房实际使用面积" v-if="typeName =='organ'">
              </el-table-column>
              <el-table-column prop="attAreaSum" label="附属用房实际建筑面积" v-if="typeName =='organ'">
              </el-table-column>
            </el-table>
          </div>

  ...
  <div>
    ...
    <div class="h-table f-relative">
      <el-table :data="list">
        ...
        <el-table-column prop="reportRentStatusName" label="是否为租（借）用"></el-table-column>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            ...
            <router-link v-show="reportOperatStatus !== 'detail'" :to="{path: '/report-manage/report/office-house-update/edit', query: {id: scope.row.id}}">修改</router-link>
    ...
    <!-- 详情 -->
    <V-officeHouseDetail ref="officeHouseDetail"></V-officeHouseDetail>
    <!--删除提示框-->
...
<script>
import officeHouseDetail from '@/components/report-manage/report/office-house-manage/detail'
import { stringify } from 'querystring';
export default {
  components: {
    'V-officeHouseDetail': officeHouseDetail
  },
  data() {
    return {
      YConfirmShow: false,
  ...
  activated() {
    ...
    this.getPic();
    this.getDataRequest();
    this.$store.commit('reportPageStatusMutations', '2');
  },
  computed: {
    dataReportOrderId() {
  ...
  methods: {
    ...
    getDataRequest() {
      ...
      this.$axiosGet(API.report_getReportOfficeHouseList, params).then(res => {
        if (res.result === 'success') {
    ...
    openDetail(id) {
      this.$refs.officeHouseDetail.openModal(id);
    },
    YConfirmClick() {
      let params = {
        id: this.id
      }
      this.$axiosPost(API.report_delReportOfficeHouse, params).then(res => {
        if (res.result == 'success') {
          this.isBol = true;
        } else {
          this.isBol = false;

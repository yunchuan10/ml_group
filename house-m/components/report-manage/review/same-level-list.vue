  ...
  <div>
    ...
    <div class="h-table f-relative">
      <el-table :data="list" empty-text='暂未发起上报' @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="reportAreaName" label="行政区划名称"></el-table-column>
        <el-table-column prop="reportAreaLevelName" label="行政区划级别"></el-table-column>
        <el-table-column prop="reportAreaCode" label="行政区划代码"></el-table-column>
        <el-table-column prop="createRealName" label="填表人"></el-table-column>
    ...
    <!-- 审核 -->
    <el-dialog title="审核" custom-class="h-dialog-add" :visible.sync="someLevelReviewDialogVisible" width="700px" @close="clearForm">
      <div>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="someLevelReviewDialogVisible = false">取 消</el-button>
...
export default {
  data() {
    return {
      ...
      message: '', // 确认信息
      someLevelReviewDialogVisible: false, // 审核弹出框
      loading: false,
      page: 1,
      pageSize: 10,
      total: 0,
  ...
  watch: {
    reportReviewPage(value) {
      if (value == 'someLevel') {
        this.someLevelReviewDialogVisible = true;
      }
    }
  },
  activated() {
    this.$store.commit('reportBackPageMutations', 'reviewSame');
    this.page = 1;
    this.getDataRequest();
  },
  methods: {
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          this.$axiosGet(API.report_auditApplyOrder, params).then(res => {
            ...
            this.someLevelReviewDialogVisible = false;
    ...
    getDataRequest() {
      ...
      this.$axiosGet(API.report_getOrganApply, params).then(res => {
        if (res.result === 'success') {

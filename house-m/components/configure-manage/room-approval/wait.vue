    ...
    <div class="h-table f-relative">
      <el-table :data="list" stripe style="width: 100%">
        ...
        <el-table-column fixed="right" label="操作">
          <template slot-scope="scope">
            <a @click="approvalInfoClick(scope.row.officeApplyId, scope.row.formKey)" v-show="scope.row.formKey != 'manageMakePlan'">审批</a>
            <a @click="approvalDetailClick(scope.row.officeApplyId)">详情</a>
            <router-link v-show="scope.row.formKey == 'manageMakePlan'" :to="{path:'/configure-manage/room-approval/allocation-scheme-first', query: {id: scope.row.officeApplyId, formKey: scope.row.formKey}}">制定调配方案</router-link>
          </template>
...
export default {
  ...
  methods: {
    ...
    getDataRequest() {
      ...
      this.$axiosPost(API.officeapply_watingApply, params).then(res => {

  ...
  <div>
    <section class="m-main">
      ...
      <ul class="house-nav-tab">
        <router-link :to="{path: '/configure-manage/room-approval/approval-list/wait'}" exact tag="li">
          <a>待处理
            <i class="f-weight-bold">（{{waitTotal}}）</i>
          </a>
        </router-link>
        <router-link :to="{path: '/configure-manage/room-approval/approval-list/plan'}" tag="li">
          <a>已处理
...
export default {
  ...
  methods: {
    getTotalApply() {
      ...
      this.$axiosPost(API.officeapply_getTotalApply, params).then(res => {

  ...
  <div>
    <section class="m-main">
      ...
      <div class="m-breadcrumb">
        <Y-Breadcrumb title="不动产证" :item="['权属管理','不动产证']" type="1"></Y-Breadcrumb>
        <div class="u-bread-btn-group">
          <router-link to="/ownership-manage/immovables-certificate/update">
      ...
      <div class="h-table f-relative">
        <el-table :data="list" stripe style="width: 100%">
          ...
          <el-table-column label="登记日期" prop="registerDate">
            <template slot-scope="scope">{{checkTime(scope.row.registerDate)}}</template>
          </el-table-column>
          <el-table-column label="发证机关" prop="licenceIssuingAuthority"></el-table-column>
          ...
          <el-table-column label="操作">
            <template slot-scope="scope" >
              ...
              <router-link :to="{path: '/ownership-manage/immovables-certificate/update', query: {estateCertificateId:scope.row.id}}">修改</router-link>
...
<script>
import detail from '@/components/ownership-manage-v2/immovables-certificate/detail'
export default {
  components: {
    'V-Detail': detail
  },
  ...
  methods: {
    ...
    getDataRequest() {
      ...
      this.$axiosPost(API.estateCertificate_estateCertificateList, params).then(res => {
        if (res.data) {
          let list = [];
          list = res.data.data || [];
    ...
    getEchartData() {
      ...
      this.$axiosPost(API.estateCertificate_estateCertificateChartList, params).then(res => {
    ...
    YDeleteClick() {
      ...
      this.$axiosPost(API.estateCertificate_doDelete, params).then(res => {

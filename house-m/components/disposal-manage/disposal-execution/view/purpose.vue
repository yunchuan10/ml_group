  ...
  <div>
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <div class="grid-content u-grid-content f-relative f-clearfix">
            <el-tooltip class="item u-grid-tips" effect="dark" content="指所有订单中最终完成用途转换的房间数量之和。" placement="right-start">
              <i class="iconfont icon-gantanhao"></i>
            </el-tooltip>
            <el-col :span="10">
              <div class="u-query-icons">
                <img v-if="!iconShowlist[0]" :src="$store.state.imgPath.disposal_transfor">
            ...
            <el-col :span="14">
              <div class="u-query-txt">
                <p>转换房间数量</p>
                <h2 class="u-query-h2">{{allPerformRoomNum || '0'}}间</h2>
              </div>
            </el-col>
          </div>
        </el-col>
        <el-col :span="8">
          <div class="grid-content u-grid-content f-relative f-clearfix">
            <el-tooltip class="item u-grid-tips" effect="dark" content="指所有订单中最终完成用途转换的房间面积之和。" placement="right-start">
              <i class="iconfont icon-gantanhao"></i>
            </el-tooltip>
            <el-col :span="10">
              <div class="u-query-icons">
            ...
            <el-col :span="14">
              <div class="u-query-txt">
                <p>转换房间面积</p>
                <h2 class="u-query-h2">{{allPerformRoomAreaNum ? allPerformRoomAreaNum: '0'}}㎡</h2>
              </div>
            </el-col>
          </div>
        </el-col>
    ...
    <div class="h-table">
      <el-table :data="list" stripe style="width: auto">
        ...
        <el-table-column prop="planPerformRoomNum" label="拟转换房间数量（间）">
        </el-table-column>
        <el-table-column prop="realPerformRoomNum" label="本次转换房间数量（间）">
        </el-table-column>
        <el-table-column prop="manageOrganName" label="管理单位">
        </el-table-column>
        <el-table-column prop="purpose" label="转换用途">
        </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      activeName: 'number',
      handleType: 'CONVERSIONPURPOSES',
      list: [], //表格列表信息    
      organList: [], //单位集合
      userId: '', //用户名id
      roomNoteList: [],
      allPerformRoomNum: '', //转换用途 数量
      allPerformRoomAreaNum: '', //转换用途面积
    };
  },
  activated() {
    this.getTotal();
  ...
  methods: {
    ...
    getTotal(){
      ...
      this.$axiosGet(API.handleapply_handlePerformTableList,{}).then(res =>{
        if(res.data){
          this.allPerformRoomNum = res.data.allPerformRoomNum || '0'; //转换用途的数量
          this.allPerformRoomAreaNum = res.data.allPerformRoomAreaNum|| '0';  // 转换用途的面积
        }   
      })
    },
    //获取列表信息

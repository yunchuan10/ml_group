  ...
  <div>
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <div class="grid-content u-grid-content f-relative f-clearfix">
            <el-tooltip class="item u-grid-tips" effect="dark" content="指所有订单中最终拍卖房间的数量之和。" placement="right-start">
              <i class="iconfont icon-gantanhao"></i>
            </el-tooltip>
            <el-col :span="10">
              <div class="u-query-icons">
                <img v-if="!iconShowlist[0]" :src="$store.state.imgPath.disposal_auction">
            ...
            <el-col :span="14">
              <div class="u-query-txt">
                <p>拍卖房间</p>
                <h2 class="u-query-h2">{{allAuctionRoomNum || '0'}}间</h2>
              </div>
            </el-col>
          </div>
        </el-col>
        <el-col :span="8">
          <div class="grid-content u-grid-content f-relative f-clearfix">
            <el-tooltip class="item u-grid-tips" effect="dark" content="指所有订单中最终拍卖房间的面积之和。" placement="right-start">
              <i class="iconfont icon-gantanhao"></i>
            </el-tooltip>
            <el-col :span="10">
              <div class="u-query-icons">
            ...
            <el-col :span="14">
              <div class="u-query-txt">
                <p>拍卖面积</p>
                <h2 class="u-query-h2">{{allAuctionRoomAreaNum ? allAuctionRoomAreaNum: '0'}}㎡</h2>
              </div>
            </el-col>
          </div>
        </el-col>
        <el-col :span="8">
          <div class="grid-content u-grid-content f-relative f-clearfix">
            <el-tooltip class="item u-grid-tips" effect="dark" content="指所有订单中拍卖房间最终成交金额之和。" placement="right-start">
              <i class="iconfont icon-gantanhao"></i>
            </el-tooltip>
            <el-col :span="10">
              <div class="u-query-icons">
            ...
            <el-col :span="14">
              <div class="u-query-txt">
                <p>成交金额</p>
                <h2 class="u-query-h2">{{allAuctionClinchDealMoney || '0'}}万元</h2>
              </div>
            </el-col>
          </div>
        </el-col>
    ...
    <div class="h-table">
      <el-table :data="list" stripe style="width: auto">
        ...
        <el-table-column prop="planAuctionRoomNum" label="拟拍卖房间数量（间）">
        </el-table-column>
        <el-table-column prop="realAuctionRoomNum" label="本次拍卖房间数量（间）">
        </el-table-column>
        <el-table-column prop="totalClinchDealMoney" label="成交总金额（万元）">
        </el-table-column>
        <el-table-column prop="patLetPerson" label="拍卖人" width='120'>
        </el-table-column>
        <el-table-column prop="auctionDate" label="拍卖日期" :formatter="dateFormat">
        </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      list: [], //表格列表信息    
      auctionDate: '', //拍卖日期
      allAuctionRoomNum: '', //拍卖
      allAuctionRoomAreaNum: '', //拍卖面积
      totalClinchDealMoney: '', //拍卖成交总金额
      allAuctionClinchDealMoney: ''
    };
  },
  activated() {
    this.getTotal();
  ...
  methods: {
    ...
    getTotal(){
      ...
      this.$axiosGet(API.handleapply_handlePerformTableList,{}).then(res =>{
        if(res.data){
          this.allAuctionRoomAreaNum = res.data.allAuctionRoomAreaNum || "0";
          this.allAuctionClinchDealMoney = res.data.allAuctionClinchDealMoney || "0";
          this.allAuctionRoomNum = res.data.allAuctionRoomNum || "0";
        }   
      })
    },
    //获取列表信息
    getDataRequest() {
      ...
      this.$axiosGet(API.handleapply_handlePerformList, params).then(res => {
        ...
        if (res.data) {
          ...
          // if(res.data.data && res.data.data[0].auctionDate){
          //   this.auctionDate = res.data.data[0].auctionDate.substr(0,10);
          // }else{
          //   this.auctionDate = ''
          // }        
        }

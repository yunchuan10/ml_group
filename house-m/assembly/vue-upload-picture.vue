...
<template>
  <div style="overflow: hidden;">
    <div v-for="(li, index) in pictureList" :key="index" class="upload-main-div" style="width: 100px; height: 98px;">
      <img :src="li.url" class="upload-img" alt="" style="max-width: 100px; max-height: 100px;" />
      <input v-if="!disabled" class="upload-btn" title='' ref="referenceUpload" type="file" :accept="accept" :id="acceptId + index" @change="picupload($event, index)" style="width: 100px; height: 100px;">
      <div class="title" v-show="!li.url" style="width: 100px; height: 100px;">
...
export default {
  ...
  props: ['pic', 'max', 'title', 'isDisabledButton', 'maxSize', 'disabled', 'width', 'height', 'name'],
  data() {
    return {
      pictureList: [{ // 已上传图片列表
        url: '',
  ...
  mounted() {
    ...
    this.$refs.referenceUpload.value = '' // 清空待上传数据
  ...
  methods: {
    picupload(event, index) { // 向后端获取upToken    
      ...
      if (file) {     
        ...
        if (isName) {
          self.$axiosPost(API.flie_getUploadToken, {}).then(res => {
            let token = res.data.token
            self.upUrl = res.data.upUrl
            formData['token'] = token
            formData['key'] = `${res.data.fileName}.${fileSuffix}`
            self.picuploadPhoto(formData, index, fileNames)
            this.$refs.referenceUpload.value = '';  
          })
        } else {
          return false
    ...
    picuploadPhoto(formData, index, fileNames) { // 上传图片至七牛
      this.$axiosUpload('https://up.qbox.me/', formData).then(res => {
        if (res.key !== null && res.key !== undefined) {
          ...
          this.pictureList[index].name = fileNames;
          if (this.pictureList.length < this.maxNumber) {
            this.pictureList.push({ name: '', url: '', key: Date.now() })
          }
        }
    ...
    getPic(val) { // 接收到父组件数据
      ...
      if (val !== null && val !== undefined) {
        if (val.length > 0) {
          ...
          for (let value of val) {
            if (value) {
              this.pictureList.push({ name: value.name, url: value.url, key: Date.now() })
            }
          }
          this.$refs.referenceUpload.value = '' // 清空待上传数据
        }       
      ...
      if (this.pictureList.length < this.maxNumber) {
        if (!this.disabled) this.pictureList.push({ name: '', url: '', key: Date.now() })
      }
    }
  },
  watch: {

  ...
  <div>
    <div class="h-table">
      <div class="u-grid-chart f-relative">
        <div class="u-grid-select">
          <el-form>
            <el-form-item label="当前位置">
              <el-select v-model="selectedOption" placeholder="请输入院落名称" @change="getSwitch">
                <el-option v-for="item in courtyardList" :key="item.value" :label="item.label" :value="item.value"></el-option>
              </el-select>
            </el-form-item>
          </el-form>
        </div>
        ...
        <div class="u-grid-charttab">
          <el-radio-group v-model="activeName" @change="getSwitch" style="margin-bottom: 30px;" size="small">
          ...
          <tr>
            <td>
              <ul class="f-clearfix">
                <li>
                  <span>使用状态</span>
                  <el-select v-model="houseStatus" filterable clearable placeholder="请选择使用状态">
                    <el-option v-for="item in houseStatusList" :key="item.dicValue" :label="item.dicName" :value="item.dicValue"></el-option>
                  </el-select>
                </li>
                <!-- <li>
                  <span>房间类型</span>
                  <el-cascader :options="roomTypeList" v-model="houseType" filterable clearable placeholder="请选择房间类型">
                  </el-cascader>
                </li> -->
              </ul>
            </td>
      ...
      <el-table :data="list" stripe style="width: auto">
        ...
        <el-table-column label="当前使用人">
          <template slot-scope="scope">
            <span v-show="scope.row.useUsers.length > 0" v-for="(item,index) in scope.row.useUsers" :key="index">
              <!-- <a @click="linkPlanImg(item.userId)" target="_block">{{item.realName}}</a> -->
              <a target="_blank">{{item.realName}}</a>
            </span>
          </template>
        </el-table-column>
      </el-table>
...
<script>
// import roomDetail from '@/components/basic-information/query-manage/view/roomcompany-detail'; // 详情弹出框
    ...
    return {
      ...
      chatOptions: {
        ...
        series: [{
          type: 'pie',
          radius: ['40%', '60%'],
          color: ['#ffd016', '#584cd8', '#b069f8', '#409eff', '#00b875', '#ff6400'],
          data: [],
          label: {
            normal: {}
          }
  ...
  methods: {
    ...
    getRoomCount() {
      let params = {
        ...
        courtyardId: this.selectedOption
      }
      this.$axiosPost(API.organSearch_getTable1Chart, params).then(res => {
        if (res.data) {
    ...
    getTableData() {
      let params = {
        courtyardId: this.selectedOption,
        organId: this.organId,
        roomTypeIds: JSON.stringify(this.houseType), //房间使用类型
        useClientStatus: this.houseStatus, //房间使用状态
        currentPage: this.page,
        pageSize: this.pageSize
      }
      this.$axiosPost(API.organSearch_roomList, params).then(res => {
        if (res.data) {
          let list = [];
          list = res.data.data;
    ...
    getSelectRequest() {
      ...
      this.$axiosPost(API.organSearch_table1Data, params).then(res => {
        if (res.data) {
          this.houseStatusList = res.data.houseStatusList;
          this.roomTypeList = res.data.roomTypeList || [];
          this.courtyardList = res.data.courtyardList || [];
          if (this.courtyardList.length > 0) {
            this.selectedOption = this.courtyardList[0].value;
          }
        } else {
    ...
    roomDetailInfoClick(roomId) {
      this.$refs.roomDetail.openModal(roomId, 'company');
    },

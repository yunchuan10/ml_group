<template>
  <div>
    <el-dialog title="新增房间" custom-class="h-dialog-add" :visible.sync="dialogShow" width="700px" @close="clearForm">
      <div class="h-tab-content" style="margin-top: 0px;">        
        ...
        <el-form-item label="房间位置" prop="roomNo">
          <el-form-item prop="roomPos">
              <el-input clearable v-model="ruleForm.roomPos" placeholder="请输入房间位置"></el-input>
          </el-form-item>
        </el-form-item>
        ...
        <el-form-item label="建筑面积" prop="useArea">
...
export default {
  ...
  methods: {
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          let params = {
            ...
            userArea: this.ruleForm.useArea,
            imgUrls: JSON.stringify(this.ruleForm.domains),
          }
          if (this.isAdd) {
            url = API.room_add;
          } else {
            url = API.room_update;
            this.$set(params, 'roomId', this.roomID)
          }
          this.loading = true;
    ...
    getDetail() {
      ...
      this.$axiosPost(API.room_updateDetail, params).then(res => {
        this.roomCheckList = [];
        this.ruleForm.roomNumber = res.data.roomNo || "";
        this.ruleForm.buildArea = res.data.buildArea || '';
        this.ruleForm.useArea = res.data.userArea;

<template>
  ...
  <div class="canvas-wraper f-relative">
        ...
        <div class="plane-box f-clearfix">
          ...
          <div v-show="!isDetail" class="plane-box-info">           
            <table class="box-info-tab">
              ...
              <tr><td>使用面积</td><td>{{roomData.useArea ? roomData.useArea+'㎡': ''}}</td></tr>
              <tr><td>房屋类型</td><td>{{roomData.roomTypeNameString ? roomData.roomTypeNameString : ''}}</td></tr>
              <tr><td>使用人数</td><td>{{roomData.userNum ? roomData.userNum+'人': '0'}}</td></tr>
              <tr><td>房间名称</td><td>{{roomData.name }}</td></tr>
              <tr><td>使用状态</td><td>{{roomData.useClientStatusName}}</td></tr>
              <tr><td>使用人</td><td><span v-for ="(item,index) in roomData.useUserList" :key="index"> <span v-show="index > 0">，</span>{{ item.realName }}</span></td></tr>
              <tr><td>照片信息</td><td> 
    ...
    <e-vue-contextmenu ref="ctxshow" id="contextStyle">
      <div class="vue-menu-icon">
        <el-tooltip class="item" effect="dark" content="分配房间" placement="bottom">
          <el-button class="context-btn"  :disabled="writeAble == 'NO'?true:false"  @click="allocationRoom">
            <i class="iconfont icon-fenpei1"></i> 
          </el-button>
        </el-tooltip>
        <el-tooltip class="item" effect="dark" content="取消分配" placement="bottom">
          <el-button class="context-btn"  @click="cancelClick"  :disabled="disableMenu || (writeAble == 'NO')">
            <i class="iconfont icon-quxiaofenpei"></i>
          </el-button>
        </el-tooltip>
      </div>
    ...
    <!--删除提示框-->
    <Y-Confirm message="您确定要取消分配嘛？" title="取消分配" v-model="YConfirmShow" @click="getCancel"></Y-Confirm>
    <!--操作提示框-->
...
export default{
  ...
  activated() {
    ...
    if(this.$route.query.writeAble){
      this.writeAble =this.$route.query.writeAble
  ...
  methods:{
    ...
    updateCourtyardName(data){
      if(data.typeFlag ==='storeys'){
        ...
        if (data.fileUrl) {
          this.floorPlanImg = data.fileUrl;
          this.lastImg = this.floorPlanImg;
          this.fabricObj.setBackgroundImage(data.fileUrl, this.fabricObj.renderAll.bind(this.fabricObj), {
            repeat:'no-repeat',
            originX: 'left',
            originY: 'top',
          });
    ...
    initCavObj(roomInfo, cavInfo) {
      if (cavInfo) {
        ...
        let group = new fabric.Group([fabricObject, roomNo], {
          ...
          useClientStatus: roomInfo.useClientStatus,
    ...
    //分配房间
    allocationRoom() {
      this.$refs.allocation.openModal(this.roomId, this.$route.query.organId);
    },
    //取消分配操作
    getCancel() {
      let params = {
      ...
      this.upperCanvas.oncontextmenu = (event) => {
        ...
        for (let i = objects.length - 1; i >= 0; i--) {
          ...
          if (this.fabricObj.containsPoint(event, object)) {
            ...
            //显示菜单
            this.selectOneObj = object; 
            if(this.selectOneObj.useClientStatus == 'USE'){

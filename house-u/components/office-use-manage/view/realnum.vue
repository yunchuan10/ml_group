  ...
  <div>
        ...
        <tr>
          <td>
            <ul class="f-clearfix" @keydown="$keydown($event, getSearch)">
              <li>
                <span>职务级别</span>
                <el-select v-model="dutiesLevelId" filterable clearable placeholder="请选择职务级别">
    ...
    <div class="h-table">
      <el-table :data="list" stripe style="width: auto">
        ...
        <el-table-column label="超标情况" width='150' fixed="right" :render-header="renderExceedingTip">
          <template slot-scope="scope">
            <span class="z-state" :class="scope.row.excessiveArea?'s-state-prohibited':'s-state-success'">{{ scope.row.excessiveArea ?'超标':'不超标'}}</span>
          </template>
        </el-table-column>
      </el-table>
    </div>
  ...
  methods: {
    ...
    getDataRequest() {
      ...
      this.$axiosPost(API.useSummary_getUserRoomAssignBase, params).then(res => {
        if (res.data) {
          // this.dutiesLevelList = res.data.data.dutiesLevel;
          let list = [];
          list = res.data.data || [];

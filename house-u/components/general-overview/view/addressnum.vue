        ...
        <div class="h-tab-content">
          <table class="h-table-info">
            ...
            <tr>
              ...
              <th>实有人数</th>
              <td>
                <a @click="personDetailInfoClick(generalOrganId)">{{realUserNum ? realUserNum : 0}}人</a>
              </td>
            </tr>
            <tr>
              <th>办公地址</th>
          ...
          <div class="h-tab-content">
            <table class="h-table-info">
              ...
              <tr v-for="(item, index) in roomList" :key="index">
                <td align="center">
                  <a @click="roomDetailInfoClick(generalOrganId,currentAddress,item.roomTypeName,item.roomTypeId)">{{item.roomTypeName}}</a>
...
<script>
import roomDetail from './addressnum-overview-detail'; // 详情弹出框
import personDetail from './addressnum-person-detail'; // 人员信息汇总表弹出框
export default {
  components: {
    ...
    getTableData() {
      ...
      this.$axiosGet(API.organSearch2_table1Data, params).then(res => {
        ...
        if (res.data) {
          ...
          this.baseRoomUseAreaSum = list.baseRoomUseAreaSum; //基本办公用房面积

        ...
        <div class="h-tab-content">
          ...
          <el-table :data="list" stripe style="width: 100%">
            ...
            <el-table-column label="房间号" prop="roomNo">
              <template slot-scope="scope">
                <a @click="roomDetailInfoClick(scope.row.roomId,organId)">{{scope.row.roomNo}}</a>
              </template>
            </el-table-column>
            ...
            <el-table-column label="使用单位" prop="useOrganName">
            </el-table-column>
            <el-table-column label="房间类型" prop="roomBaseTypes">
            </el-table-column>
            <el-table-column label="使用状态" prop="useClientStatusName">
            </el-table-column>
            <el-table-column label="使用人数" prop="useUserNum">
            </el-table-column>
...
<script>
import roomDetail from '@/components/general-overview/view/room-sub-detail'; // 详情弹出框
export default {
  components: {
  ...
  methods: {
    ...
    getSearchDate() {
      ...
      this.$axiosPost(API.yardSearch2_getTable1Select, params).then(res => {
    ...
    getDetail() {
      ...
      this.$axiosPost(API.yardSearch2_organUseRoomList, params).then(res => {
        let list = [];
        if (res.data) {
          list = res.data.data;
    ...
    // 详情
    roomDetailInfoClick(id, gid) {
      this.$refs.roomDetail.openModal(id, gid);
    },

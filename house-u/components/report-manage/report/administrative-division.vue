<template>
  <div>
    <div class="h-tab-content">
      <el-form ref="ruleForm" :model="ruleForm" :disabled="disableds">
      ...
      <div class="table-bottom-btns f-hidden" v-show="reportOperatStatus !== 'detail'">
        <router-link to="/report-manage/report-list">
          <el-button v-if="Button===true" class="u-submit-btn cancel-btn">取消</el-button>
        </router-link>
        <el-button v-if="Button===true" class="u-submit-btn submit-btn" :loading="loading" @click="submitClick">保存</el-button>
        <el-button v-if="Button===false" class="u-submit-btn submit-btn" :loading="loading" @click="modifyClick">修改</el-button>
      </div>
    </div>
    <!--操作提示框-->
...
export default {
  ...
  methods: {
    Buttons() {
      setTimeout(() => {
        this.ruleForm.reportUserName ? this.Button = false : this.Button = true;
        this.ruleForm.reportUserName ? this.disableds = true : this.disableds = false;
      }, 500);    },
    // 获取地区初始列表

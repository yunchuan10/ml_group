        ...
        <el-form label-position="right" label-width="100px" style="width:80%;" :model="ruleForm" ref="ruleForm">
          ...
          <el-form-item label="发送对象" prop="sendObeject" :rules="{required: true, validator: sendObejectPass, trigger: 'change'}">
            <el-radio-group v-model="ruleForm.sendObeject">
              ...
              <!-- <el-radio label="外单位">外单位</el-radio> -->
            </el-radio-group>
            <el-transfer v-if="ruleForm.sendObeject == '本单位'" :props="{key: 'value'}" filterable filter-placeholder="请输入人员姓名" :titles="['待选', '已选']" v-model="ruleForm.toUserIds" :data="userList"></el-transfer>
            <el-transfer v-else :props="{key: 'value'}" filterable filter-placeholder="请输入单位名称" :titles="['待选', '已选']" v-model="ruleForm.toOrganIds" :data="organList"></el-transfer>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    sendObejectPass(rule, value, callback) {
      if (value == '本单位') {
        if (this.ruleForm.toUserIds.length > 0) {
          callback();
        } else {
          callback(new Error('发送人员不能为空'))
        }
      } else {
    ...
    getSaveDataRequest(noticeInfoStatus) {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          if (this.ruleForm.sendObeject == '外单位') {
            this.ruleForm.toUserIds = [];
          } else {
            this.ruleForm.toOrganIds = [];
          }
          let params = {
            ...
            toUserIds: this.ruleForm.toUserIds,
            toOrganIds: this.ruleForm.toOrganIds,
    ...
    YPromptClick() {
      if (this.isBol) {
        if (this.noticeInfoStatus == 'DRAFT') {
          ...
          this.$router.push({ path: '/notice-manage/notice-list' })
        }
      }
    }
  }

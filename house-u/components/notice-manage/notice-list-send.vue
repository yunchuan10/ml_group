  ...
  <div>
    <section class="m-main">
      ...
      <div class="m-breadcrumb">
        <Y-Breadcrumb  v-if="$route.path !='/notice-manage/notice-list-send/draft'" title="我发布的" :item="['我发布的']" type="1"></Y-Breadcrumb>
         <Y-Breadcrumb v-if="$route.path ==='/notice-manage/notice-list-send/draft'" title="草稿箱" :item="['草稿箱']" type="1"></Y-Breadcrumb>
        <div class="u-bread-btn-group">
          <router-link to="/notice-manage/notice-list">
      ...
      <div class="h-table f-relative">
        <el-table :data="list"  >
          ...
          <el-table-column prop="publishPeople" key="publishPeople" v-if="noticeStatus == 'PUBLISHED'" label="发送人数"></el-table-column>
          <el-table-column prop="readTimes" key="readTimes" v-if="noticeStatus == 'PUBLISHED'" label="查看人数"></el-table-column>
          <el-table-column prop="dateCreated" key="dateCreated" v-if="noticeStatus == 'DRAFT'" label="生成时间">
            <template slot-scope="scope">
              {{getTime(scope.row.dateCreated)}}
            </template>
          </el-table-column>
          <el-table-column prop="publishTime" key="publishTime" v-if="noticeStatus == 'PUBLISHED'" label="发布时间">
            <template slot-scope="scope">
    ...
    <!--删除提示框-->
    <Y-Confirm v-model="YConfirmShow" :message="message" @click="YConfirmClick"></Y-Confirm>
    <!--操作提示框-->
...
export default {
  ...
  activated() {
    if (this.$route.params.type == 'published') {
      this.noticeStatus = 'PUBLISHED';
    }
    if(this.$route.params.type == 'draft'){
      this.noticeStatus = 'DRAFT';
    }   
    this.getDataRequest();
  },
  methods: {
    ...
    getDataRequest() {
      ...
      this.$axiosPost(API.publish_myPublish, params).then(res => {

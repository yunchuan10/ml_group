    ...
    <section class="m-main">
      ...
      <div class="m-breadcrumb">
        ...
        <div class="u-bread-btn-group">
          <button class="f-pull-right u-bread-btn" @click="bindCharacterDialog = true">
            批量绑定
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getSearchDate() {
      ...
      this.$axiosGet(API.person_findOrganDutiesByOrganId, params).then(res => {
        ...
        if (valid) {
          ...
          this.$axiosPost(API.person_bindRole, params).then(res => {
            if (res.result == 'success') {
              this.isBol = true;
              if (this.userIdList) {
                BURYINGPOINT.buryingPoint('通用模块', '角色权限', '批量绑定角色')
              } else {
